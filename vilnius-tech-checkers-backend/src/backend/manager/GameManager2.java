...
public class GameManager2 implements SubscriptionSupport, RulesetService {
    ...
    public GameManager2(CheckersRuleset ruleset) {
    ...
    public void processMove(Move move) {
       ...
       // eventEmitter.emit(new MoveProcessedEvent(move));
        ...
        if(arguments.isSwitchSide()) {
            var index = getPlayingSides().indexOf(currentSide);
            currentSide = getPlayingSides().get((index + 1) % getPlayingSides().size());
        }
